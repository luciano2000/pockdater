...
Pockdater is a CLI tool for updating openFPGA cores, firmware, and assets on an Analogue Pocket handheld device. It runs in both an interactive menu mode and a non-interactive CLI mode.

## Commands

```bash
# Restore dependencies
dotnet restore pockdater.csproj

# Build
dotnet build pockdater.csproj --no-restore

# Run tests
dotnet test pockdater.csproj --no-build --verbosity normal

# Run a specific test (xUnit)
dotnet test pockdater.csproj --filter "FullyQualifiedName~TestName"

# Run the app
dotnet run --project pockdater.csproj -- [verb] [options]

# Publish for a target platform
dotnet publish pockdater.csproj -r osx-arm64 --self-contained true -c Release -o ./out
```

There is also `pockdater_legacy.csproj` targeting .NET Framework — keep changes compatible with it when possible.
...
- `src/services/` — Core business logic. The main orchestrator is `CoreUpdaterService`. Other notable services:
  ...
  - `SettingsService` — reads/writes `pockdater_settings.json`
...
- **Namespace root:** `Pockdater.*`
- **JSON serialization:** Newtonsoft.Json; model property names use `snake_case` to match device file formats
- **Settings file:** `pockdater_settings.json` next to the executable; managed by `SettingsService`
