...
## Improvements over pupdate

- **Atomic downloads**: Files are downloaded to a `.tmp` file and only renamed to the final name when the download completes successfully. Interrupted downloads no longer leave corrupt files on disk.
- **Retry logic**: Download failures are automatically retried up to 3 times with exponential backoff (1s, 2s, 4s delays), making the tool resilient to transient network errors.
- **Fixed file handle leak in ZipHelper**: `FileStream` and `ZipArchive` are now properly wrapped in `using` blocks, preventing file handle leaks when extraction fails mid-way.
- **Backup-restore for all video.json writes**: `ChangeAspectRatio` and `ClearDisplayModes` now use the same backup-restore pattern as `AddDisplayModes`, preventing corruption of `video.json` on partial writes (issue #354 mitigation extended).
- **Atomic instance JSON writes**: `BuildInstanceJson` writes to a `.tmp` file first, then renames atomically, avoiding corrupt output JSON files on failure.
- **Configurable download timeout**: The download timeout (previously hardcoded to 100 seconds) is now configurable via `download_timeout_seconds` in `pockdater_settings.json` (default: 100).
...
Use **Settings** from the main menu to walk through toggles, or edit `pockdater_settings.json` for advanced options.
...
- **Set Archive Cache Location** — When **Cache downloaded archive files locally** is enabled, this overrides the default cache folder under your user **LocalApplicationData** `pockdater\cache` (Windows) / equivalent. Leave empty (or save an empty line) to use that default. Same as `config.archive_cache_location`.
- **Set Temp Directory** — Where pockdater extracts downloads and temporary files. Empty means the OS temp folder (`Path.GetTempPath()`). Same as `config.temp_directory`.

### Set GitHub Token

Stores a [personal access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) for GitHub API calls (higher rate limits than anonymous requests). The current value is shown when you open this item (same idea as **Patreon Config → Set Patreon Email Address**). You can also set `config.github_token` in `pockdater_settings.json`.
...
Toggles exposed in the **Settings** menu (stored in `pockdater_settings.json`):
...
| Delete cores, not managed by pockdater | Remove unmanaged cores from SD when enabled |
...
| Automatically install updates to Pockdater | Self-update without prompt on startup |
...
Edit `pockdater_settings.json` for keys that are not bool menu toggles:
...
The `archives` array in `pockdater_settings.json` defines where assets come from. Stock config includes a default **archive.org** archive and a **Retrodriven** custom archive; you can switch via **Use custom archive** or edit JSON.
...
Some items need login. Add to `pockdater_settings.json`:
...
```
  menu                     Interactive main menu (default)
    ...
    -s, --skip-update         Open menu without checking for pockdater self-update
  ...
  update-self              Check for pockdater updates
...
```text
pockdater -p /path/to/sdcard/
pockdater update -c boogermann.bankpanic
pockdater assets -c jotego.jtcontra
pockdater images -i pocket-platform-images -o dyreschlock -v home
```
...
Pockdater can download the latest `jtbeta.zip` for you. **Manual placement (putting `jtbeta.zip` on the SD root yourself) always wins** — auto-fetch only runs when no manual key is present. Two optional sources are supported; if both are enabled, GitHub is tried first and Patreon is the fallback.
...
4. Set `github_token` in `pockdater_settings.json` to that PAT.
...
4. In pockdater: **Pocket Setup → Patreon Config → Set Patreon Session Cookie**, paste the value.
...
- Your subscription/access must actually grant beta privileges — pockdater will tell you if it doesn't.
...
- **`Error in framework RS: bridge not responding`** — Run pockdater on a local disk, then copy results to the SD card.
...
- **GitHub API rate limit** — Add a token via **Pocket Setup → Set GitHub Token** or `config.github_token` in `pockdater_settings.json`.

---

## Developers

- **.NET** — `dotnet restore`, `dotnet build`, `dotnet run --project pockdater.csproj -- [verb] [options]`.
- **Layout** — `src/Program.cs`, `src/partials/`, `src/services/`, `src/models/`, `src/options/`.
- **Legacy** — `pockdater_legacy.csproj` targets .NET Framework where applicable.
