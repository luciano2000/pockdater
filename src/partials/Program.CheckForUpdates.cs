...
internal static partial class Program
{
    ...
    {
        Console.WriteLine("pockdater v" + VERSION);
        Console.WriteLine("Checking for updates...");

        if (CheckVersion(path) && !selfUpdate)
        {
#if NET7_0
            Console.WriteLine("You are using a legacy version of Pockdater that is running on .NET 7.0");
            Console.WriteLine("Auto/Self updates are not supported in this mode.");
            Console.WriteLine("Please download the latest version of Pockdater from GitHub directly.");
