...
public class HttpHelper
{
    ...
    private const int DOWNLOAD_MAX_RETRIES = 3;

    /// <summary>
    /// Default timeout in seconds for DownloadFile calls that do not specify an explicit timeout.
    /// Can be overridden via pockdater_settings.json (download_timeout_seconds).
    /// </summary>
    public int DefaultDownloadTimeout { get; set; } = 100;

    public void DownloadFile(string uri, string outputPath, int timeout = -1)
    {
        DownloadFileWithRetry(uri, outputPath, timeout < 0 ? this.DefaultDownloadTimeout : timeout);
    }

    private void DownloadFileWithRetry(string uri, string outputPath, int timeout)
    {
        ...
        for (int attempt = 1; attempt <= DOWNLOAD_MAX_RETRIES; attempt++)
        {
            if (attempt > 1)
            {
                ...
                int delaySeconds = 1 << (attempt - 2);
                Console.WriteLine($"\rRetrying download (attempt {attempt}/{DOWNLOAD_MAX_RETRIES}) in {delaySeconds}s...");
                Thread.Sleep(TimeSpan.FromSeconds(delaySeconds));
            }

            try
            {
                this.DownloadFileOnce(uri, outputPath, timeout);
                return; // Success
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    Console.WriteLine($"\rDownload attempt {attempt} failed: {ex.Message}");
                }
            }
        }

        throw new Exception($"Download failed after {DOWNLOAD_MAX_RETRIES} attempts: {uri}", lastException);
    }

    private void DownloadFileOnce(string uri, string outputPath, int timeout)
    {
        bool console = false;
        ...
        // Download to a temporary file first; only rename on success to avoid leaving
        // a corrupt partial file at the final destination if the download is interrupted.
        ...
        // Atomically replace the destination file with the successfully downloaded temp file
