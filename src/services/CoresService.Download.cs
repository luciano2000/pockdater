...
using Pockdater.Exceptions;
using Pockdater.Helpers;
using Pockdater.Models.Analogue.Data;
using Pockdater.Models.Analogue.Instance;
using Pockdater.Models.Analogue.Instance.Simple;
using Pockdater.Models.Events;
using Pockdater.Models.InstancePackager;
using Pockdater.Models.OpenFPGA_Cores_Inventory.V3;
using Pockdater.Models.Settings;
...
using InstancePackagerDataSlot =  Pockdater.Models.InstancePackager.DataSlot;
            ...
            && ((archive.one_time && !archive.complete) || !archive.one_time))
        {
            ...
            foreach (var file in files)
            {
                ...
                else
                {
                    ...
                    {
                        // Extract and only delete the archive if extraction succeeds
        ...
        foreach (string dir in Directory.GetDirectories(commonPath, "*", SearchOption.AllDirectories))
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Write atomically: write to a temp file first, then rename to the final path
                    // to avoid leaving a corrupt partial file if the process is interrupted.
                    string tempOutputFile = outputFile + ".tmp";

                    try
                    {
                        File.WriteAllText(tempOutputFile, json);
