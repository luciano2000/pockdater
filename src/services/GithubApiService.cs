...
using Pockdater.Models.Github;
using GithubFile = Pockdater.Models.Github.File;
...
public static class GithubApiService
{
    ...
    public static byte[] DownloadFileContents(string user, string repository, string path, string githubToken = null)
    {
        ...
        request.Headers.UserAgent.Add(new ProductInfoHeaderValue("Pockdater", "1.0"));
    ...
    private static string CallApi(string url, string token)
    {
        ...
        var agent = new ProductInfoHeaderValue("Pockdater", "1.0");
