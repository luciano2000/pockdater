...
using Pockdater.Models.Analogue.Video;
using AnalogueDisplayMode = Pockdater.Models.Analogue.Video.DisplayMode;
using DisplayMode = Pockdater.Models.DisplayModes.DisplayMode;
...
public partial class CoresService
{
    ...
    public void ChangeAspectRatio(string identifier, int fromWidth, int fromHeight, int toWidth, int toHeight)
    {
        ...
        WriteVideoJsonSafe(identifier, json);
    }
        ...
        bool forceOriginal = false, bool merge = false)
    {
        ...
        // Use the shared backup-restore helper to protect against partial writes (issue #354)
        WriteVideoJsonSafe(identifier, json);
    }
    ...
    /// <summary>
    /// Writes video.json with a backup-restore pattern to protect against partial writes.
    /// Mirrors the protection used in AddDisplayModes (issue #354).
    /// </summary>
    private void WriteVideoJsonSafe(string identifier, string json)
    {
